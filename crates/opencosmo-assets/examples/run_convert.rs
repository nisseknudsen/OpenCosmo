...
fn main() -> Result<()> {
    ...
    let did_convert = opencosmo_assets::convert::convert_all_episodes_if_stale(&sh_path, &out_dir)?;
    println!("converted={did_convert} in {:?}", start.elapsed());
    let start = std::time::Instant::now();
    let did_convert2 = opencosmo_assets::convert::convert_all_episodes_if_stale(&sh_path, &out_dir)?;
