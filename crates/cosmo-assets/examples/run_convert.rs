...
fn main() -> Result<()> {
    ...
    let did_convert = cosmo_assets::convert::convert_episode1_if_stale(&sh_path, &out_dir)?;
    println!("converted={did_convert} in {:?}", start.elapsed());
    let start = std::time::Instant::now();
    let did_convert2 = cosmo_assets::convert::convert_episode1_if_stale(&sh_path, &out_dir)?;
